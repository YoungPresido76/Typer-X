...
import {
  Trophy, Calendar, CheckCircle, Zap, Award,
    ...
    <div className="space-y-6">
      ...
      <div className="flex bg-[#121417] p-1 rounded-xl border border-neutral-800 max-w-md mx-auto">
        {([
          ...
          >
            {t.label}
            {t.id === activeSubTab && pendingClaim > 0 && (
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-[#121417] rounded-xl border border-neutral-800 max-w-md mx-auto">
        ...
        <p className="text-[10px] text-gray-600 font-mono">
          {activeSubTab === 'daily'
            ? `Daily missions reset at midnight · ${filtered.filter(m => !m.claimed).length} remaining`
            : activeSubTab === 'weekly'
            ? 'Weekly missions reset every Monday'
            : 'Achievements are permanent — they never reset'
          }
        </p>
      </div>
        ...
        <div className="flex items-start gap-3 p-4 bg-orange-500/5 border border-orange-500/10 rounded-2xl max-w-md mx-auto">
          ...
          <div>
            ...
            <p className="text-[10px] text-gray-500 mt-0.5 leading-relaxed font-mono">
              Missions are assigned and waiting. Once you install the Typer X Android keyboard,
              progress syncs here automatically as you type in any app.
            </p>
          </div>
        </div>
      )}
                ...
                <div className="flex-1 space-y-2 w-full">
                  <div className="flex items-center gap-2">
                    <div className={`p-2 rounded-xl border ${
                      ...
                      : m.completed ? 'bg-orange-950/10 text-orange-400 border-orange-500/15'
                      ...
                      <div className="flex-1 h-2 bg-neutral-900 rounded-full overflow-hidden border border-neutral-800">
                        <div
                          className={`h-full rounded-full transition-all duration-300 ${
                            m.claimed ? 'bg-gradient-to-r from-emerald-500 to-teal-500'
