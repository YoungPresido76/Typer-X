...
import { Flame, Zap, Target, Award, ArrowUpRight, Trophy, Star, ChevronRight } from 'lucide-react';
...
export const DashboardTab: React.FC<DashboardTabProps> = ({ userStats, missions, onNavigate }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#1A1D22] p-5 rounded-2xl border border-neutral-850 flex flex-col md:flex-row justify-between items-center gap-4">
        <div className="flex items-center gap-3.5">
          {/* Circular level indicator with double gradient */}
          ...
          <div>
            <h3 className="text-[15px] font-bold text-white flex items-center gap-1.5 leading-tight">
              Level {userStats.level} Pilot <span className="text-[10px] px-1.5 py-0.5 rounded bg-orange-500/10 text-orange-400 font-mono">ELITE TYPIST</span>
            </h3>
        ...
        {/* Day Streak Card with Flame and active triggers */}
        <div className="w-full md:w-auto p-3.5 bg-gradient-to-r from-orange-500/15 to-orange-500/5 rounded-xl border border-orange-500/15 flex items-center justify-between md:justify-around gap-4">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 bg-[#121417] rounded-xl flex items-center justify-center border border-orange-500/20 text-orange-400">
              <Flame className="w-5.5 h-5.5 fill-orange-500 animate-pulse" />
            </div>
            <div>
              <span className="text-xs font-black text-white tracking-wide block">DAY STREAK BOOST</span>
              <p className="text-[10px] text-orange-400/80 font-mono">Maintain activity streak to lock multipliers</p>
            </div>
          </div>
      ...
      {/* 2. Today's Performance Core stats bento grid */}
      <div>
        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Today's Performance Overview</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          
          {/* Card A: Words typed */}
          <div className="bg-[#1A1D22] p-4.5 rounded-2xl border border-neutral-850 flex flex-col justify-between">
            ...
            <div className="mt-4">
              ...
              <p className="text-[10px] text-gray-500 font-mono mt-1">Cumulative sandbox logs</p>
            </div>
          </div>
          ...
          <div className="bg-[#1A1D22] p-4.5 rounded-2xl border border-neutral-850 flex flex-col justify-between">
            ...
            <div className="mt-4">
              <h2 className="text-2xl font-black text-white">{userStats.avgWpm} <span className="text-xs text-neutral-500">WPM</span></h2>
      ...
      {/* 3. Daily Goals Progress & Active Missions list */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        
        {/* Daily Goal card */}
        <div className="bg-[#1A1D22] p-5 rounded-2xl border border-neutral-850 flex flex-col justify-between">
          <div>
            ...
            <p className="text-[10px] text-gray-500 mb-4 font-mono">Type words inside the mobile simulator to advance!</p>
          </div>

          <div className="space-y-4">
            <div className="w-full h-2.5 bg-neutral-900 rounded-lg overflow-hidden border border-neutral-850">
              <div 
            ...
            <div className="flex justify-between text-[11px] font-mono text-gray-500">
              ...
              <span>Reward: <span className="text-orange-400">+300 XP Bonus</span></span>
            </div>
          </div>
        </div>

        {/* Daily Missions Small List overview */}
        <div className="bg-[#1A1D22] p-5 rounded-2xl border border-neutral-850 flex flex-col justify-between">
          <div className="flex justify-between items-center mb-3.5">
            <h3 className="text-xs font-bold text-white uppercase tracking-wide flex items-center gap-1.5">
              <Trophy className="w-4 h-4 text-orange-500" /> Core Daily Missions ({missions.filter(m => m.type === 'daily' && m.completed).length}/{activeMissions.length})
            </h3>
            <span 
              onClick={() => onNavigate('missions')}
              className="text-[10px] text-orange-400 cursor-pointer font-bold hover:underline flex items-center"
            >
          ...
          <div className="space-y-3">
            {activeMissions.map((m) => {
              ...
              return (
                <div key={m.id} className="p-3 bg-[#121417] rounded-xl border border-neutral-850 flex items-center justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <span className="text-xs font-bold text-gray-200 block truncate">{m.title}</span>
                  ...
                  <div>
                    {m.completed ? (
                      <span className="text-[9px] px-2 py-1 rounded-md bg-emerald-950/30 text-emerald-400 border border-emerald-500/20 font-bold font-mono">COMPLETED</span>
                    ) : (
